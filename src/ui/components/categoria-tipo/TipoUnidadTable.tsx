        ...
        {loading ? (
          <div className="flex justify-center items-center py-12">
            <div className="bg-gray-100/50 backdrop-blur-sm rounded-2xl p-8">
              ...
              <p className="mt-4 text-base font-medium text-gray-600 text-center">Cargando tipos de unidad...</p>
            </div>
          </div>
        ) : tiposUnidad.length === 0 ? (
          <div className="flex justify-center items-center py-12">
            <div className="bg-gray-100/50 backdrop-blur-sm rounded-3xl p-8 border border-gray-200/50 text-center">
              <p className="text-gray-700 text-xl font-medium">No hay tipos de unidad registrados</p>
              <p className="text-gray-500 text-base font-light mt-2">Comienza agregando tu primer tipo de unidad</p>
            </div>
          </div>
        ) : (
          <Table>
            <TableHeader>
              <TableRow className="border-gray-200/50 hover:bg-gray-50/50">
                ...
                <TableHead className="text-gray-600 font-light">Abreviación</TableHead>
            ...
            <TableBody>
              {tiposUnidad.map((tipo) => (
                <TableRow key={tipo.id} className="border-gray-200/50 hover:bg-gray-50/50 transition-colors">
                  <TableCell className="py-4">
                    <div className="font-light text-gray-900">{tipo.nombre}</div>
                  </TableCell>
                  <TableCell className="py-4">
                    <Badge className="px-3 py-1 rounded-xl text-xs font-medium bg-blue-50/80 text-blue-700 border border-blue-200/50">
                      {tipo.abreviacion}
                    </Badge>
                  </TableCell>
                  <TableCell className="py-4">
                  ...
                  <TableCell className="py-4">
                    <Badge 
                      variant={tipo.activo ? "success" : "destructive"}
