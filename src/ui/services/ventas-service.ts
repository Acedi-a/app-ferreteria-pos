...
export class VentasService {
  ...
  static async obtenerVentas(filtros: FiltrosVenta = {}, limite: number = 100): Promise<Venta[]> {
    try {
      ...
      const params: any[] = [cajaActiva.id];

      // Excluir ventas canceladas por defecto, a menos que se especifique el filtro de estado
      if (!filtros.estado) {
  ...
  static async obtenerEstadisticasVentas(): Promise<any> {
    try {
      ...
      const ventasHoy = await window.electronAPI.db.get(`
        ...
        FROM ventas 
        WHERE DATE(fecha_venta) = DATE(?) AND estado != 'cancelada' AND caja_id = ?
      ...
      const ventasSemana = await window.electronAPI.db.get(`
        SELECT 
          ...
          COALESCE(SUM(total), 0) as total
        FROM ventas 
        WHERE DATE(fecha_venta) >= DATE('now', '-7 days') AND estado != 'cancelada' AND caja_id = ?
      `, [cajaActiva.id]);
      ...
      const ventasMes = await window.electronAPI.db.get(`
        SELECT 
          ...
          COALESCE(SUM(total), 0) as total
        FROM ventas 
        WHERE strftime('%Y-%m', fecha_venta) = strftime('%Y-%m', 'now') AND estado != 'cancelada' AND caja_id = ?
      `, [cajaActiva.id]);
      ...
      const topProductos = await window.electronAPI.db.query(`
        ...
        WHERE DATE(v.fecha_venta) >= DATE('now', '-30 days') AND v.estado != 'cancelada' AND v.caja_id = ?
    ...
    static async cancelarVenta(ventaId: number, motivo?: string): Promise<void> {
        ...
        // Registrar transacción de cancelación en caja (egreso para revertir el ingreso)
            try {
                ...
                console.log(`Venta #${ventaId} cancelada exitosamente`);

            } catch (error) {
                // Revertir todos los cambios si algo falló
                await window.electronAPI.db.run('ROLLBACK');
